...
export default function PropertyFeatures({ metadata }: { metadata: IPropertyMetadata }) {
  return (
    <div className="flex w-full flex-col items-start gap-[45px] md:w-1/2">
      <div className="space-y-4 text-[1.125rem]/[1.5rem]">
        ...
        <p className="font-sans text-[#717171] ">{metadata.propertyDescription || ''}</p>
      </div>
      <div className="w-full space-y-2.5">
        <h1 className="font-mona text-[1.125rem]/[1.5rem] font-semibold text-[#4E4E4E]">
          Details
        </h1>
        <PropertyInfoDetails title="Blocks" value={metadata.attributes?.area || ''} />
        <PropertyInfoDetails
          title="Bedrooms"
          value={metadata.attributes?.numberOfBedrooms || ''}
        />
        <PropertyInfoDetails
          title="Bathrooms"
          value={metadata.attributes?.numberOfBathrooms || ''}
        />
        <PropertyInfoDetails title="Type" value={metadata.propertyType} />
        <PropertyInfoDetails
          title="Location"
          value={`${metadata.address.street}, ${metadata.address.townCity}`}
        />
      </div>
    </div>
  );
