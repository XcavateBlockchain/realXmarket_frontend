...
const ddbClient = new DynamoDBClient({ region: process.env.AWS_REGION, credentials: {accessKeyId : process.env.AWS_ACCESS_KEY!, secretAccessKey:process.env.AWS_SECRET_KEY! } });
const ddbDocClient = DynamoDBDocumentClient.from(ddbClient);

export async function upsertProperty(accountAddress: string, propertyId: number, propertyData: any) {
  const params = {
...
export async function addFileToProperty(accountAddress: string, propertyId: number, fileKey: string) {
  const params = {
    TableName: 'real-marketplace-properties',
